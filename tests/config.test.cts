...
function readConfig(tmpDir: string) {
  const configPath = path.join(tmpDir, '.planning', 'config.json');
  return JSON.parse(fs.readFileSync(configPath, 'utf-8'));
}

function writeConfig(tmpDir: string, obj: unknown) {
  const configPath = path.join(tmpDir, '.planning', 'config.json');
...
describe('config-ensure-section command', () => {
  ...
  test('creates config.json with expected structure and types', () => {
    const result = runVectorTools('config-ensure-section', tmpDir);
  ...
  test('is idempotent — returns already_exists on second call', () => {
    const first = runVectorTools('config-ensure-section', tmpDir);
    assert.ok(first.success, `First call failed: ${first.error}`);
    const firstOutput = JSON.parse(first.output);
    assert.strictEqual(firstOutput.created, true);

    const second = runVectorTools('config-ensure-section', tmpDir);
  ...
  test('detects Brave Search from file-based key', () => {
    ...
    const vectorDir = path.join(homedir, '.vector');
    const braveKeyFile = path.join(vectorDir, 'brave_api_key');
    ...
    const vectorDirExisted = fs.existsSync(vectorDir);
    try {
      if (!vectorDirExisted) {
      ...
      if (!vectorDirExisted) {
        try { fs.rmdirSync(vectorDir); } catch { /* ignore if not empty */ }
      }
    }
  });
  ...
  test('merges user defaults from defaults.json', () => {
    const homedir = os.homedir();
    const vectorDir = path.join(homedir, '.vector');
    const defaultsFile = path.join(vectorDir, 'defaults.json');
    ...
    try {
      ...
      if (!vectorDirExisted) {
        try { fs.rmdirSync(vectorDir); } catch { /* ignore */ }
      }
    }
  });
...
describe('config-set command', () => {
  ...
  beforeEach(() => {
    ...
    runVectorTools('config-ensure-section', tmpDir);
  });
  ...
  test('sets a top-level string value', () => {
    const result = runVectorTools('config-set model_profile quality', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
  ...
  test('coerces true to boolean', () => {
    const result = runVectorTools('config-set commit_docs true', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
  ...
  test('coerces false to boolean', () => {
    const result = runVectorTools('config-set commit_docs false', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
  ...
  test('coerces numeric strings to numbers', () => {
    const result = runVectorTools('config-set granularity 42', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
  ...
  test('preserves plain strings', () => {
    const result = runVectorTools('config-set model_profile hello', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
  ...
  test('sets nested values via dot-notation', () => {
    const result = runVectorTools('config-set workflow.research false', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
  ...
  test('rejects unknown config keys', () => {
    const result = runVectorTools('config-set workflow.nyquist_validation_enabled false', tmpDir);
  ...
  test('errors when no key path provided', () => {
    const result = runVectorTools('config-set', tmpDir);
    assert.strictEqual(result.success, false);
  });
...
describe('config-get command', () => {
  ...
  test('gets a top-level value', () => {
    const result = runVectorTools('config-get model_profile', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
  ...
  test('gets a nested value via dot-notation', () => {
    const result = runVectorTools('config-get workflow.research', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
  ...
  test('errors for nonexistent key', () => {
    const result = runVectorTools('config-get nonexistent_key', tmpDir);
    assert.strictEqual(result.success, false);
    assert.ok(
  ...
  test('errors for deeply nested nonexistent key', () => {
    const result = runVectorTools('config-get workflow.nonexistent', tmpDir);
    assert.strictEqual(result.success, false);
    assert.ok(
      result.error!.includes('Key not found'),
      `Expected "Key not found" in error: ${result.error!}`
  ...
  test('errors when config.json does not exist', () => {
    ...
    try {
      const result = runVectorTools('config-get model_profile', emptyTmpDir);
      assert.strictEqual(result.success, false);
      assert.ok(
  ...
  test('errors when no key path provided', () => {
    const result = runVectorTools('config-get', tmpDir);
    assert.strictEqual(result.success, false);
  });
});
