...
function writeMinimalRoadmap(tmpDir: string, phases = ['1']) {
  const lines = phases.map((n: any) => `### Phase ${n}: Phase ${n} Description`).join('\n');
  fs.writeFileSync(
...
function writeMinimalProjectMd(tmpDir: string, sections = ['## What This Is', '## Core Value', '## Requirements']) {
  const content = sections.map((s: any) => `${s}\n\nContent here.\n`).join('\n');
  fs.writeFileSync(
...
function writeValidConfigJson(tmpDir: string) {
  fs.writeFileSync(
...
describe('validate health command', () => {
  ...
  test("returns 'broken' when .planning directory is missing", () => {
    ...
    const result = runVectorTools('validate health', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    const output = JSON.parse(result.output);
    assert.strictEqual(output.status, 'broken', 'should be broken');
    assert.ok(
      output.errors.some((e: any) => e.code === 'E001'),
  ...
  test('warns when PROJECT.md is missing', () => {
    ...
    const output = JSON.parse(result.output);
    assert.ok(
      output.errors.some((e: any) => e.code === 'E002'),
  ...
  test('warns when PROJECT.md missing required sections', () => {
    ...
    const output = JSON.parse(result.output);
    const w001s = output.warnings.filter((w: any) => w.code === 'W001');
    assert.ok(w001s.length > 0, `Expected W001 warnings: ${JSON.stringify(output.warnings)}`);
    assert.ok(
      w001s.some((w: any) => w.message.includes('## Core Value')),
  ...
  test('passes when PROJECT.md has all required sections', () => {
    ...
    assert.ok(
      !output.warnings.some((w: any) => w.code === 'W001'),
  ...
  test('errors when ROADMAP.md is missing', () => {
    ...
    const output = JSON.parse(result.output);
    assert.ok(
      output.errors.some((e: any) => e.code === 'E003'),
  ...
  test('errors when STATE.md is missing with repairable true', () => {
    ...
    const output = JSON.parse(result.output);
    const e004 = output.errors.find((e: any) => e.code === 'E004');
  ...
  test('warns when STATE.md references nonexistent phase', () => {
    ...
    const output = JSON.parse(result.output);
    assert.ok(
      output.warnings.some((w: any) => w.code === 'W002'),
  ...
  test('warns when config.json is missing with repairable true', () => {
    ...
    const output = JSON.parse(result.output);
    const w003 = output.warnings.find((w: any) => w.code === 'W003');
  ...
  test('errors when config.json has invalid JSON', () => {
    ...
    const output = JSON.parse(result.output);
    assert.ok(
      output.errors.some((e: any) => e.code === 'E005'),
  ...
  test('warns when config.json has invalid model_profile', () => {
    ...
    const output = JSON.parse(result.output);
    assert.ok(
      output.warnings.some((w: any) => w.code === 'W004'),
  ...
  test('warns about incorrectly named phase directories', () => {
    ...
    const output = JSON.parse(result.output);
    assert.ok(
      output.warnings.some((w: any) => w.code === 'W005'),
  ...
  test('reports orphaned plans (PLAN without SUMMARY) as info', () => {
    ...
    const output = JSON.parse(result.output);
    assert.ok(
      output.info.some((i: any) => i.code === 'I001'),
  ...
  test('warns about phase in ROADMAP but not on disk', () => {
    ...
    const output = JSON.parse(result.output);
    assert.ok(
      output.warnings.some((w: any) => w.code === 'W006'),
  ...
  test('warns about phase on disk but not in ROADMAP', () => {
    ...
    const output = JSON.parse(result.output);
    assert.ok(
      output.warnings.some((w: any) => w.code === 'W007'),
  ...
  test('detects W008 when workflow.nyquist_validation absent from config', () => {
    ...
    const output = JSON.parse(result.output);
    assert.ok(
      output.warnings.some((w: any) => w.code === 'W008'),
  ...
  test('detects W009 when RESEARCH.md has Validation Architecture but no VALIDATION.md', () => {
    ...
    const output = JSON.parse(result.output);
    assert.ok(
      output.warnings.some((w: any) => w.code === 'W009'),
...
describe('validate health --repair command', () => {
  ...
  test('creates config.json with defaults when missing', () => {
    ...
    const result = runVectorTools('validate health --repair', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    const createAction = output.repairs_performed.find((r: any) => r.action === 'createConfig');
    ...
    assert.strictEqual(diskConfig.phase_branch_template, 'vector/phase-{phase}-{slug}');
    assert.strictEqual(diskConfig.milestone_branch_template, 'vector/{milestone}-{slug}');
  });
  ...
  test('resets config.json when JSON is invalid', () => {
    ...
    const resetAction = output.repairs_performed.find((r: any) => r.action === 'resetConfig');
  ...
  test('regenerates STATE.md when missing', () => {
    ...
    const regenerateAction = output.repairs_performed.find((r: any) => r.action === 'regenerateState');
  ...
  test('backs up existing STATE.md before regenerating', () => {
    ...
    const backupFile = planningFiles.find((f: any) => f.startsWith('STATE.md.bak-'));
  ...
  test('adds nyquist_validation key to config.json via addNyquistKey repair', () => {
    ...
    const addKeyAction = output.repairs_performed.find((r: any) => r.action === 'addNyquistKey');
