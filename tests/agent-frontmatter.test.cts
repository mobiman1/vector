...
describe('SPAWN: spawn type consistency', () => {
  test('no "First, read agent .md" workaround pattern remains', () => {
    ...
    for (const dir of dirs) {
      ...
      const files = fs.readdirSync(dir).filter((f: any) => f.endsWith('.md'));
  ...
  test('diagnose-issues uses vector-debugger (not general-purpose)', () => {
    const content = fs.readFileSync(
      path.join(WORKFLOWS_DIR, 'diagnose-issues.md'), 'utf-8'
    );
    assert.ok(
      content.includes('subagent_type="vector-debugger"'),
      'diagnose-issues should spawn vector-debugger, not general-purpose'
    );
  });
});
