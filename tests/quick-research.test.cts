...
describe('quick command: --research in frontmatter', () => {
  ...
  test('quick.md exists', () => {
    assert.ok(fs.existsSync(commandPath), 'commands/vector/quick.md should exist');
  });
  ...
  test('argument-hint includes all three flags', () => {
    content = fs.readFileSync(commandPath, 'utf-8');
    const hintLine = content.split('\n').find((l: any) => l.includes('argument-hint'));
...
describe('quick workflow: research step', () => {
  ...
  test('research step spawns vector-phase-researcher', () => {
    content = fs.readFileSync(workflowPath, 'utf-8');
    ...
    assert.ok(
      researchSection.includes('subagent_type="vector-phase-researcher"'),
      'research step should spawn vector-phase-researcher agent'
    );
  });
