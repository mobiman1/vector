...
`;
    ...
    assert.ok(
      digest.decisions.some((d: any) => d.decision === 'Use Prisma over Drizzle'),
...
describe('list-todos command', () => {
  ...
  test('returns multiple todos with correct fields', () => {
    ...
    const testTodo = output.todos.find((t: any) => t.file === 'add-tests.md');
...
`
    ...
    assert.strictEqual(
      stats.phases.find((p: any) => p.number === '16')?.name,
      'Multi-Claim Verification & UX'
    );
    assert.strictEqual(
      stats.phases.find((p: any) => p.number === '16')?.status,
      'Not Started'
    );
  });
