...
describe('state-snapshot command', () => {
  ...
  test('missing STATE.md returns error', () => {
    const result = runVectorTools('state-snapshot', tmpDir);
...
`
    ...
    const outsideDir = fs.mkdtempSync(path.join(require('os').tmpdir(), 'vector-test-outside-'));

    try {
      const result = runVectorTools(`state-snapshot --cwd "${tmpDir}"`, outsideDir);
      assert.ok(result.success, `Command failed: ${result.error!}`);
  ...
  test('returns error for invalid --cwd path', () => {
    ...
    const result = runVectorTools(`state-snapshot --cwd "${invalid}"`, tmpDir);
...
`
    ...
    const result = runVectorTools(['state', 'add-blocker', '--text', 'Waiting on vendor quote $1.00 before approval'], tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
...
`
    ...
    const result = runVectorTools(`state add-blocker --text-file "${blockerPath}"`, tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
...
describe('state json command', () => {
  ...
  test('missing STATE.md returns error', () => {
    const result = runVectorTools('state json', tmpDir);
    assert.ok(result.success, `Command should succeed: ${result.error!}`);
...
`
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.vector_state_version, '1.0', 'should have version 1.0');
...
`
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.vector_state_version, '1.0', 'version from frontmatter');
...
`
    ...
    const result = runVectorTools('state update Status "Executing Plan 1"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);

    const content = fs.readFileSync(path.join(tmpDir, '.planning', 'STATE.md'), 'utf-8');
    assert.ok(content.startsWith('---\n'), 'should start with frontmatter delimiter');
    assert.ok(content.includes('vector_state_version: 1.0'), 'should have version field');
...
`
    ...
    const result = runVectorTools('state patch --Status "In progress" --"Current Plan" 04-02', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
...
`
    ...
    runVectorTools('state update Status "In progress"', tmpDir);
    runVectorTools('state update Status "Paused"', tmpDir);
...
`
    ...
    runVectorTools('state update Status "Executing Plan 5"', tmpDir);
...
describe('cmdStateLoad (state load)', () => {
  ...
  test('returns config and state when STATE.md exists', () => {
    ...
    const result = runVectorTools('state load', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
  ...
  test('returns raw key=value format with --raw flag', () => {
    ...
    const result = runVectorTools('state load --raw', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
...
describe('cmdStateGet (state get)', () => {
  ...
  test('returns full content when no section specified', () => {
    ...
    const result = runVectorTools('state get', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
  ...
  test('extracts bold field value', () => {
    ...
    const result = runVectorTools('state get Status', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
  ...
  test('extracts markdown section content', () => {
    ...
    const result = runVectorTools('state get Blockers', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
  ...
  test('returns error for nonexistent field', () => {
    ...
    const result = runVectorTools('state get Missing', tmpDir);
...
describe('cmdStatePatch and cmdStateUpdate (state patch, state update)', () => {
  ...
  test('state patch updates multiple fields at once', () => {
    ...
    const result = runVectorTools('state patch --Status Complete --"Current Phase" 04', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
  ...
  test('state patch reports failed fields that do not exist', () => {
    ...
    const result = runVectorTools('state patch --Status Done --Missing value', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
  ...
  test('state update changes a single field', () => {
    ...
    const result = runVectorTools('state update Status "Phase complete"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
  ...
  test('state update reports field not found', () => {
    ...
    const result = runVectorTools('state update Missing value', tmpDir);
  ...
  test('state update returns error when STATE.md missing', () => {
    const result = runVectorTools('state update Status value', tmpDir);
...
describe('cmdStateAdvancePlan (state advance-plan)', () => {
  ...
  test('advances plan counter when not on last plan', () => {
    ...
    const result = runVectorTools('state advance-plan', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
...
describe('cmdStateRecordMetric (state record-metric)', () => {
  ...
  test('appends metric row to existing table', () => {
    ...
    const result = runVectorTools('state record-metric --phase 2 --plan 1 --duration 5min --tasks 3 --files 4', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
  ...
  test('replaces None yet placeholder with first metric', () => {
    ...
    const result = runVectorTools('state record-metric --phase 1 --plan 1 --duration 2min --tasks 1 --files 2', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
  ...
  test('returns error when required fields missing', () => {
    ...
    const result = runVectorTools('state record-metric --phase 1', tmpDir);
    assert.ok(result.success, `Command should exit 0: ${result.error!}`);
  ...
  test('returns error when STATE.md missing', () => {
    const result = runVectorTools('state record-metric --phase 1 --plan 1 --duration 2min', tmpDir);
    assert.ok(result.success, `Command should exit 0: ${result.error!}`);
...
describe('cmdStateUpdateProgress (state update-progress)', () => {
  ...
  test('calculates progress from plan/summary counts', () => {
    ...
    const result = runVectorTools('state update-progress', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
...
describe('cmdStateResolveBlocker (state resolve-blocker)', () => {
  ...
  test('removes matching blocker line (case-insensitive substring match)', () => {
    ...
    const result = runVectorTools('state resolve-blocker --text "api credentials"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
  ...
  test('adds None placeholder when last blocker resolved', () => {
    ...
    const result = runVectorTools('state resolve-blocker --text "single blocker"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
  ...
  test('returns error when text not provided', () => {
    ...
    const result = runVectorTools('state resolve-blocker', tmpDir);
    assert.ok(result.success, `Command should exit 0: ${result.error!}`);
  ...
  test('returns error when STATE.md missing', () => {
    const result = runVectorTools('state resolve-blocker --text "anything"', tmpDir);
    assert.ok(result.success, `Command should exit 0: ${result.error!}`);
  ...
  test('returns resolved true even if no line matches', () => {
    ...
    const result = runVectorTools('state resolve-blocker --text "nonexistent blocker text"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
...
describe('cmdStateRecordSession (state record-session)', () => {
  ...
  test('updates Last session timestamp even with no other options', () => {
    ...
    const result = runVectorTools('state record-session', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
  ...
  test('sets Resume file to None when not specified', () => {
    ...
    const result = runVectorTools('state record-session --stopped-at "Phase 1 complete"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
...
describe('milestone-scoped phase counting in frontmatter', () => {
  ...
  test('total_phases counts only current milestone phases', () => {
    ...
    const result = runVectorTools('state update Status "Executing"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);

    // Read the state json to check frontmatter
    const jsonResult = runVectorTools('state json', tmpDir);
  ...
  test('without ROADMAP counts all phases (pass-all filter)', () => {
    ...
    const result = runVectorTools('state update Status "In progress"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error!}`);
