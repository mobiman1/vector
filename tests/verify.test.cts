...
describe('validate consistency command', () => {
  ...
  test('passes for consistent project', () => {
    ...
    const result = runVectorTools('validate consistency', tmpDir);
  ...
  test('warns about phase on disk but not in roadmap', () => {
    ...
    assert.ok(
      output.warnings.some((w: any) => w.includes('disk but not in ROADMAP')),
  ...
  test('warns about gaps in phase numbering', () => {
    ...
    const output = JSON.parse(result.output);
    assert.ok(
      output.warnings.some((w: any) => w.includes('Gap in phase numbering')),
...
describe('verify plan-structure command', () => {
  ...
  test('reports missing required frontmatter fields', () => {
    ...
    const result = runVectorTools('verify plan-structure .planning/phases/01-test/01-01-PLAN.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    const output = JSON.parse(result.output);
    assert.strictEqual(output.valid, false, 'should be invalid');
    assert.ok(
      output.errors.some((e: any) => e.includes('Missing required frontmatter field')),
  ...
  test('reports task missing name element', () => {
    ...
    const output = JSON.parse(result.output);
    assert.ok(
      output.errors.some((e: any) => e.includes('Task missing <name>')),
  ...
  test('reports task missing action element', () => {
    ...
    const output = JSON.parse(result.output);
    assert.ok(
      output.errors.some((e: any) => e.includes('missing <action>')),
  ...
  test('warns about wave > 1 with empty depends_on', () => {
    ...
    const output = JSON.parse(result.output);
    assert.ok(
      output.warnings.some((w: any) => w.includes('Wave > 1 but depends_on is empty')),
  ...
  test('errors when checkpoint task but autonomous is true', () => {
    ...
    const output = JSON.parse(result.output);
    assert.ok(
      output.errors.some((e: any) => e.includes('checkpoint tasks but autonomous is not false')),
      `Expected checkpoint/autonomous error in errors: ${JSON.stringify(output.errors)}`
    );
  });

  test('returns error for nonexistent file', () => {
    const result = runVectorTools('verify plan-structure .planning/phases/01-test/nonexistent.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('verify phase-completeness command', () => {
  ...
  test('reports complete phase with matching plans and summaries', () => {
    ...
    const result = runVectorTools('verify phase-completeness 01', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('reports incomplete phase with plan missing summary', () => {
    ...
    assert.ok(
      output.incomplete_plans.some((id: string) => id.includes('01-01')),
      `Expected "01-01" in incomplete_plans: ${JSON.stringify(output.incomplete_plans)}`
    );
    assert.ok(
      output.errors.some((e: any) => e.includes('Plans without summaries')),
  ...
  test('warns about orphan summaries', () => {
    ...
    const output = JSON.parse(result.output);
    assert.ok(
      output.warnings.some((w: any) => w.includes('Summaries without plans')),
      `Expected "Summaries without plans" in warnings: ${JSON.stringify(output.warnings)}`
    );
  });

  test('returns error for nonexistent phase', () => {
    const result = runVectorTools('verify phase-completeness 99', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('verify summary command', () => {
  ...
  test('returns not found for nonexistent summary', () => {
    const result = runVectorTools('verify-summary .planning/phases/01-test/nonexistent.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    assert.ok(
      output.errors.some((e: any) => e.includes('SUMMARY.md not found')),
  ...
  test('passes for valid summary with real files and commits', () => {
    ...
    const result = runVectorTools('verify-summary .planning/phases/01-test/01-01-SUMMARY.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('respects checkFileCount parameter', () => {
    ...
    const result = runVectorTools('verify-summary .planning/phases/01-test/01-01-SUMMARY.md --check-count 1', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('verify references command', () => {
  ...
  test('reports valid when all referenced files exist', () => {
    ...
    const result = runVectorTools('verify references .planning/phases/01-test/doc.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error for nonexistent file', () => {
    const result = runVectorTools('verify references .planning/phases/01-test/nonexistent.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('verify commits command', () => {
  ...
  test('validates real commit hashes', () => {
    ...
    const result = runVectorTools(`verify commits ${hash}`, tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('reports invalid for fake hashes', () => {
    const result = runVectorTools('verify commits abcdef1234567', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('handles mixed valid and invalid hashes', () => {
    ...
    const result = runVectorTools(`verify commits ${hash} abcdef1234567`, tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('verify artifacts command', () => {
  ...
  function writePlanWithArtifacts(tmpDir: string, artifactsYaml: string[]) {
  ...
  test('passes when all artifacts exist and match criteria', () => {
    ...
    const result = runVectorTools('verify artifacts .planning/phases/01-test/01-01-PLAN.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('reports missing artifact file', () => {
    ...
    assert.ok(
      output.artifacts[0].issues.some((i: any) => i.includes('File not found')),
  ...
  test('reports insufficient line count', () => {
    ...
    assert.strictEqual(output.all_passed, false, 'Expected all_passed false');
    assert.ok(
      output.artifacts[0].issues.some((i: any) => i.includes('Only') && i.includes('lines, need 10')),
  ...
  test('reports missing pattern', () => {
    ...
    assert.strictEqual(output.all_passed, false, 'Expected all_passed false');
    assert.ok(
      output.artifacts[0].issues.some((i: any) => i.includes('Missing pattern')),
  ...
  test('reports missing export', () => {
    ...
    assert.strictEqual(output.all_passed, false, 'Expected all_passed false');
    assert.ok(
      output.artifacts[0].issues.some((i: any) => i.includes('Missing export')),
...
describe('verify key-links command', () => {
  ...
  function writePlanWithKeyLinks(tmpDir: string, keyLinksYaml: string[]) {
    // parseMustHavesBlock expects 4-space indent for block name, 6-space for items, 8-space for keys
    const content = [
      '---',
      'phase: 01-test',
  ...
  test('verifies link when pattern found in source', () => {
    ...
    const result = runVectorTools('verify key-links .planning/phases/01-test/01-01-PLAN.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
