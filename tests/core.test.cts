...
describe('loadConfig', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'vector-core-test-'));
  ...
  test('returns model_overrides when present (REG-01)', () => {
    ...
    const config = loadConfig(tmpDir);
    assert.deepStrictEqual(config.model_overrides, { 'vector-executor': 'opus' });
  });
...
describe('resolveModelInternal', () => {
  ...
  describe('override precedence', () => {
    test('per-agent override takes precedence over profile', () => {
      ...
      assert.strictEqual(resolveModelInternal(tmpDir, 'vector-executor'), 'haiku');
    });
    ...
    test('opus override resolves to inherit', () => {
      ...
      assert.strictEqual(resolveModelInternal(tmpDir, 'vector-executor'), 'inherit');
    });
    ...
    test('agents not in override fall back to profile', () => {
      ...
      assert.strictEqual(resolveModelInternal(tmpDir, 'vector-planner'), 'inherit');
    });
  });
  ...
  describe('edge cases', () => {
    test('returns sonnet for unknown agent type', () => {
      writeConfig({ model_profile: 'balanced' });
      assert.strictEqual(resolveModelInternal(tmpDir, 'vector-nonexistent'), 'sonnet');
    });
