...
All notable changes to Vector will be documented in this file.
...
### Added
- **`/vector:quick --research` flag** — Spawns focused research agent before planning, composable with `--discuss` and `--full` (#317)
...
- **Programmatic `/vector:set-profile`** — runs as a script instead of LLM-driven workflow, executes in seconds instead of 30-40s
...
- `vector-tools.cjs` uses absolute paths in all install types (#820)
- Invalid `skills:` frontmatter removed from UI agent files

## [1.23.0] - 2026-03-15

### Added
- `/vector:ui-phase` + `/vector:ui-review` — UI design contract generation and retroactive 6-pillar visual audit for frontend phases (closes #986)
- `/vector:stats` — project statistics dashboard: phases, plans, requirements, git metrics, and timeline
- **Copilot CLI** runtime support — install with `--copilot`, maps Claude Code tools to GitHub Copilot tools
- **`vector-autonomous` skill** for Codex runtime — enables autonomous Vector execution
...
### Changed
- `/vector:discuss-phase` supports explicit `--batch` mode for grouped question intake

### Fixed
- `/vector:new-milestone` no longer resets `workflow.research` config during milestone transitions
- `/vector:update` is runtime-aware and targets the correct runtime directory
...
### Added
- `--discuss` flag for `/vector:quick` — lightweight pre-planning discussion to gather context before quick tasks
...
- Missing `skills` frontmatter on vector-nyquist-auditor agent
...
- Context monitor hook is advisory instead of blocking non-Vector workflows
...
- Code-aware discuss phase with codebase scouting — `/vector:discuss-phase` now analyzes relevant source files before asking questions
...
- `/vector:add-tests` command for post-phase test generation
- Codex runtime support with skills-first installation
- Standard `project_context` block in vector-verifier output
...
- `/vector:update` always installs latest package version
...
### Fixed
- `/vector:health --repair` now creates timestamped backup before regenerating STATE.md (#657)
...
### Added
- `/vector:health` command — validates `.planning/` directory integrity with `--repair` flag for auto-fixing config.json and STATE.md
- `--full` flag for `/vector:quick` — enables plan-checking (max 2 iterations) and post-execution verification on quick tasks
- `--auto` flag wired from `/vector:new-project` through the full phase chain (discuss → plan → execute)
- Auto-advance chains phase execution across full milestones when `workflow.auto_advance` is enabled

### Fixed
- Plans created without user context — `/vector:plan-phase` warns when no CONTEXT.md exists, `/vector:discuss-phase` warns when plans already exist (#253)
- OpenCode installer converts `general-purpose` subagent type to OpenCode's `general`
- `/vector:complete-milestone` respects `commit_docs` setting when merging branches
- Phase directories tracked in git via `.gitkeep` files

## [1.19.2] - 2026-02-15

### Added
- User-level default settings via `~/.vector/defaults.json` — set Vector defaults across all projects
...
- ESM conflicts prevented by renaming vector-tools.js to .cjs
...
### Added
- `--auto` flag for `/vector:new-project` — runs research → requirements → roadmap automatically after config questions. Expects idea document via @ reference (e.g., `/vector:new-project --auto @prd.md`)
...
- Research decision from `/vector:new-milestone` now persists to config.json

## [1.17.0] - 2026-02-08

### Added
- **vector-tools verification suite**: `verify plan-structure`, `verify phase-completeness`, `verify references`, `verify commits`, `verify artifacts`, `verify key-links` — deterministic structural checks
- **vector-tools frontmatter CRUD**: `frontmatter get/set/merge/validate` — safe YAML frontmatter operations with schema validation
- **vector-tools template fill**: `template fill summary/plan/verification` — pre-filled document skeletons
- **vector-tools state progression**: `state advance-plan`, `state update-progress`, `state record-metric`, `state add-decision`, `state add-blocker`, `state resolve-blocker`, `state record-session` — automates STATE.md updates
- **Local patch preservation**: Installer now detects locally modified Vector files, backs them up to `vector-local-patches/`, and creates a manifest for restoration
- `/vector:reapply-patches` command to merge local modifications back after Vector updates

### Changed
- Agents (executor, planner, plan-checker, verifier) now use vector-tools for state updates and verification instead of manual markdown parsing
- `/vector:update` workflow now notifies about backed-up local patches and suggests `/vector:reapply-patches`
...
### Added
- 10 new vector-tools CLI commands that replace manual AI orchestration of mechanical operations:
...
### Changed
- Workflows now delegate deterministic operations to vector-tools CLI, reducing token usage and errors:
...
### Fixed
- Execute-phase now correctly spawns `vector-executor` subagents instead of generic task agents
- `commit_docs=false` setting now respected in all `.planning/` commit paths (execute-plan, debugger, reference docs all route through vector-tools CLI)
...
- Windows: Normalized backslash paths in vector-tools invocations (contributed by @rmindel)
...
### Added
- Context-optimizing parsing commands in vector-tools (`phase-plan-index`, `state-snapshot`, `summary-extract`) — reduces agent context usage by returning structured JSON instead of raw file content
...
### Added
- `vector-tools history-digest` — Compiles phase summaries into structured JSON for faster context loading
- `vector-tools phases list` — Lists phase directories with filtering (replaces fragile `ls | sort -V` patterns)
- `vector-tools roadmap get-phase` — Extracts phase sections from ROADMAP.md
- `vector-tools phase next-decimal` — Calculates next decimal phase number for insert operations
- `vector-tools state get/patch` — Atomic STATE.md field operations
- `vector-tools template select` — Chooses summary template based on plan complexity
...
- Agents migrated from bash patterns to structured vector-tools commands
...
- **Centralized utilities** — New `vector-tools.cjs` (11 functions) replaces repetitive bash patterns across 50+ files
...
### Added
- `vector-tools.cjs` CLI utility with functions: state load/update, resolve-model, find-phase, commit, verify-summary, generate-slug, current-timestamp, list-todos, verify-path-exists, config-ensure-section
...
### Fixed
- **CRITICAL:** Prevent API keys from being committed via `/vector:map-codebase` (#429)
...
- Auto-create `config.json` when missing during `/vector:settings` (#264)
- `/vector:update` respects local vs global install location
...
- Removed broken vector-gemini link (404)
...
- Git branching strategy configuration with three options:
  ...
  - `phase`: create branch per phase (`vector/phase-{N}-{slug}`)
  - `milestone`: create branch per milestone (`vector/{version}-{slug}`)
...
### Fixed
- CONTEXT.md from `/vector:discuss-phase` now properly flows to all downstream agents (researcher, planner, checker, revision loop)
...
### Removed
- `/vector:whats-new` command — use `/vector:update` instead (shows changelog with cancel option)
...
### Added
- `/vector:join-discord` command to quickly access the Vector Discord community invite link

## [1.9.8] - 2025-01-22

### Added
- Uninstall flag (`--uninstall`) to cleanly remove Vector from global or local installations
...
- OpenCode commands use flat structure (`command/vector-help.md`) matching OpenCode's expected format
...
- Native OpenCode support: `--opencode` flag converts Vector to OpenCode format automatically
- `--both` flag to install for both Claude Code and OpenCode in one command
- Auto-configures `~/.opencode.json` permissions for seamless Vector doc access
...
- Removed stray backticks in `/vector:new-project` output
...
- **Codebase Intelligence System** — Removed due to overengineering concerns
  - Deleted `/vector:analyze-codebase` command
  - Deleted `/vector:query-intel` command
  - Removed SQLite graph database and sql.js dependency (21MB)
  - Removed intel hooks (vector-intel-index.js, vector-intel-session.js, vector-intel-prune.js)
...
### Added
- **Model Profiles** — `/vector:set-profile` for quality/balanced/budget agent configurations
- **Workflow Settings** — `/vector:settings` command for toggling workflow behaviors interactively
...
- All hooks now use `vector-` prefix for consistency (statusline.js → vector-statusline.js)
...
- `/vector:new-project` now asks about git tracking during initial setup, letting you opt out of committing planning docs from the start
...
### Added
- **Quick Mode** (`/vector:quick`) — Execute small, ad-hoc tasks with Vector guarantees but skip optional agents (researcher, checker, verifier). Quick tasks live in `.planning/quick/` with their own tracking in STATE.md.

### Changed
- Improved progress bar calculation to clamp values within 0-100 range
- Updated documentation with comprehensive Quick Mode sections in help.md, README.md, and Vector-STYLE.md
...
- Removed dead vector-researcher agent references
...
- Orphaned `vector-notify.sh` hook from previous versions is now automatically removed during install (both file and settings.json registration)

## [1.6.3] - 2025-01-17

### Added
- `--gaps-only` flag for `/vector:execute-phase` — executes only gap closure plans after verify-work finds issues, eliminating redundant state discovery
...
### Changed
- Installer performs clean install of Vector folders, removing orphaned files from previous versions
- `/vector:update` shows changelog and asks for confirmation before updating, with clear warning about what gets replaced

## [1.6.0] - 2026-01-17

### Changed
- **BREAKING:** Unified `/vector:new-milestone` flow — now mirrors `/vector:new-project` with questioning → research → requirements → roadmap in a single command
- Roadmapper agent now references templates instead of inline structures for easier maintenance

### Removed
- **BREAKING:** `/vector:discuss-milestone` — consolidated into `/vector:new-milestone`
- **BREAKING:** `/vector:create-roadmap` — integrated into project/milestone flows
- **BREAKING:** `/vector:define-requirements` — integrated into project/milestone flows
- **BREAKING:** `/vector:research-project` — integrated into project/milestone flows

### Added
- `/vector:verify-work` now includes next-step routing after verification completes
...
- Next-step suggestions now consistently recommend `/vector:discuss-phase` before `/vector:plan-phase` across all routing paths
...
- Removed blocking notification popups (vector-notify) on all platforms
...
- **BREAKING:** Removed `/vector:execute-plan` command (use `/vector:execute-phase` instead)
...
- Removed `/vector:research-phase` from next-step suggestions (use `/vector:plan-phase` instead)

## [1.5.22] - 2025-01-16

### Added
- Statusline update indicator — shows `⬆ /vector:update` when a new version is available
...
### Added
- Vector brand system for consistent UI (checkpoint boxes, stage banners, status symbols)
- Research synthesizer agent that consolidates parallel research into SUMMARY.md

### Changed
- **Unified `/vector:new-project` flow** — Single command now handles questions → research → requirements → roadmap (~10 min)
- Simplified README to reflect streamlined workflow: new-project → plan-phase → execute-phase
- Added optional `/vector:discuss-phase` documentation for UI/UX/behavior decisions before planning
...
### Changed
- `/vector:discuss-phase` redesigned with intelligent gray area analysis — analyzes phase to identify discussable areas (UI, UX, Behavior, etc.), presents multi-select for user control, deep-dives each area with focused questioning
...
- `/vector:plan-phase` now integrates research — spawns `vector-phase-researcher` before planning unless research exists or `--skip-research` flag used

## [1.5.18] - 2026-01-16

### Added
- **Plan verification loop** — Plans are now verified before execution with a planner → checker → revise cycle
  - New `vector-plan-checker` agent (744 lines) validates plans will achieve phase goals
...
- **Dedicated planner agent** — `vector-planner` (1,319 lines) consolidates all planning expertise
...
### Changed
- `/vector:plan-phase` refactored to thin orchestrator pattern (310 lines)
  - Spawns `vector-planner` for planning, `vector-plan-checker` for verification
  - User sees status between agent spawns (not a black box)
- Planning references deprecated with redirects to `vector-planner` agent sections
  - `plan-format.md`, `scope-estimation.md`, `goal-backward.md`, `principles.md`
  - `workflows/plan-phase.md`

### Fixed
- Removed zombie `vector-milestone-auditor` agent (was accidentally re-added after correct deletion)
...
### Added
- New `/vector:update` command — check for updates, install, and display changelog of what changed (better UX than raw `npx vector`)

## [1.5.16] - 2026-01-15

### Added
- New `vector-researcher` agent (915 lines) with comprehensive research methodology, 4 research modes (ecosystem, feasibility, implementation, comparison), source hierarchy, and verification protocols
- New `vector-debugger` agent (990 lines) with scientific debugging methodology, hypothesis testing, and 7+ investigation techniques
- New `vector-codebase-mapper` agent for brownfield codebase analysis
- Research subagent prompt template for context-only spawning

### Changed
- `/vector:research-phase` refactored to thin orchestrator — now injects rich context (key insight framing, downstream consumer info, quality gates) to vector-researcher agent
- `/vector:research-project` refactored to spawn 4 parallel vector-researcher agents with milestone-aware context (greenfield vs v1.1+) and roadmap implications guidance
- `/vector:debug` refactored to thin orchestrator (149 lines) — spawns vector-debugger agent with full debugging expertise
- `/vector:new-milestone` now explicitly references MILESTONE-CONTEXT.md

### Deprecated
- `workflows/research-phase.md` — consolidated into vector-researcher agent
- `workflows/research-project.md` — consolidated into vector-researcher agent
- `workflows/debug.md` — consolidated into vector-debugger agent
- `references/research-pitfalls.md` — consolidated into vector-researcher agent
- `references/debugging.md` — consolidated into vector-debugger agent
- `references/debug-investigation.md` — consolidated into vector-debugger agent

## [1.5.15] - 2025-01-15

### Fixed
- **Agents now install correctly** — The `agents/` folder (vector-executor, vector-verifier, vector-integration-checker, vector-milestone-auditor) was missing from npm package, now included

### Changed
- Consolidated `/vector:plan-fix` into `/vector:plan-phase --gaps` for simpler workflow
- UAT file writes now batched instead of per-response for better performance

## [1.5.14] - 2025-01-15

### Fixed
- Plan-phase now always routes to `/vector:execute-phase` after planning, even for single-plan phases

## [1.5.13] - 2026-01-15

### Fixed
- `/vector:new-milestone` now presents research and requirements paths as equal options, matching `/vector:new-project` format
...
- `progress` now correctly routes to `/vector:discuss-milestone` when between milestones (Route F)
...
### Added
- Milestone audit system (`/vector:audit-milestone`) for verifying milestone completion with parallel verification agents
...
- Execute-phase now recommends `/vector:audit-milestone` when milestone completes
...
### Changed
- `vector-executor` subagent color changed from red to blue

## [1.5.7] - 2025-01-15

### Added
- `vector-executor` subagent: Dedicated agent for plan execution with full workflow logic built-in
- `vector-verifier` subagent: Goal-backward verification that checks if phase goals are actually achieved (not just tasks completed)
...
### Changed
- execute-plan and execute-phase now spawn `vector-executor` subagent instead of using inline workflow
...
### Fixed
- Updated remaining `general-purpose` subagent references to use `vector-executor`
...
### Removed
- Dead `/vector:status` command (referenced abandoned background agent model)
...
### Added
- New `/vector:research-project` command for pre-roadmap ecosystem research — spawns parallel agents to investigate stack, features, architecture, and pitfalls before you commit to a roadmap
- New `/vector:define-requirements` command for scoping v1 requirements from research findings — transforms "what exists in this domain" into "what we're building"
...
### Added
- New `/vector:whats-new` command shows changes since your installed version
...
### Added
- `/vector:debug` command for systematic debugging with persistent state
...
### Added
- Parallel phase execution via `/vector:execute-phase`
- Parallel-aware planning in `/vector:plan-phase`
- `/vector:status` command for parallel agent monitoring
...
### Added
- `/vector:add-todo` and `/vector:check-todos` for mid-session idea capture
...
### Added
- `/vector:resume-task` for resuming interrupted subagent executions
...
### Added
- `/vector:verify-work` for conversational UAT validation
- `/vector:plan-fix` for fixing UAT issues
- UAT issues template
...
### Added
- `/vector:map-codebase` documentation in help and README

## [1.3.0] - 2025-12-17

### Added
- `/vector:map-codebase` command for brownfield project analysis
...
- Brownfield integration into Vector workflows
...
- `/vector:research-phase` for niche domain ecosystem discovery
- `/vector:research-project` command with workflow and templates
- `/vector:create-roadmap` command with research-aware workflow
...
[Unreleased]: https://github.com/glittercowboy/core/compare/v1.24.0...HEAD
[1.24.0]: https://github.com/glittercowboy/core/releases/tag/v1.24.0
[1.23.0]: https://github.com/glittercowboy/core/releases/tag/v1.23.0
[1.22.4]: https://github.com/glittercowboy/core/releases/tag/v1.22.4
[1.22.3]: https://github.com/glittercowboy/core/releases/tag/v1.22.3
[1.22.2]: https://github.com/glittercowboy/core/releases/tag/v1.22.2
[1.22.1]: https://github.com/glittercowboy/core/releases/tag/v1.22.1
[1.22.0]: https://github.com/glittercowboy/core/releases/tag/v1.22.0
[1.21.1]: https://github.com/glittercowboy/core/releases/tag/v1.21.1
[1.21.0]: https://github.com/glittercowboy/core/releases/tag/v1.21.0
[1.20.6]: https://github.com/glittercowboy/core/releases/tag/v1.20.6
[1.20.5]: https://github.com/glittercowboy/core/releases/tag/v1.20.5
[1.20.4]: https://github.com/glittercowboy/core/releases/tag/v1.20.4
[1.20.3]: https://github.com/glittercowboy/core/releases/tag/v1.20.3
[1.20.2]: https://github.com/glittercowboy/core/releases/tag/v1.20.2
[1.20.1]: https://github.com/glittercowboy/core/releases/tag/v1.20.1
[1.20.0]: https://github.com/glittercowboy/core/releases/tag/v1.20.0
[1.19.2]: https://github.com/glittercowboy/core/releases/tag/v1.19.2
[1.19.1]: https://github.com/glittercowboy/core/releases/tag/v1.19.1
[1.19.0]: https://github.com/glittercowboy/core/releases/tag/v1.19.0
[1.18.0]: https://github.com/glittercowboy/core/releases/tag/v1.18.0
[1.17.0]: https://github.com/glittercowboy/core/releases/tag/v1.17.0
[1.16.0]: https://github.com/glittercowboy/core/releases/tag/v1.16.0
[1.15.0]: https://github.com/glittercowboy/core/releases/tag/v1.15.0
[1.14.0]: https://github.com/glittercowboy/core/releases/tag/v1.14.0
[1.13.0]: https://github.com/glittercowboy/core/releases/tag/v1.13.0
[1.12.1]: https://github.com/glittercowboy/core/releases/tag/v1.12.1
[1.12.0]: https://github.com/glittercowboy/core/releases/tag/v1.12.0
[1.11.2]: https://github.com/glittercowboy/core/releases/tag/v1.11.2
[1.11.1]: https://github.com/glittercowboy/core/releases/tag/v1.11.0
[1.10.1]: https://github.com/glittercowboy/core/releases/tag/v1.10.1
[1.10.0]: https://github.com/glittercowboy/core/releases/tag/v1.10.0
[1.9.12]: https://github.com/glittercowboy/core/releases/tag/v1.9.12
[1.9.11]: https://github.com/glittercowboy/core/releases/tag/v1.9.11
[1.9.10]: https://github.com/glittercowboy/core/releases/tag/v1.9.10
[1.9.9]: https://github.com/glittercowboy/core/releases/tag/v1.9.9
[1.9.8]: https://github.com/glittercowboy/core/releases/tag/v1.9.8
[1.9.7]: https://github.com/glittercowboy/core/releases/tag/v1.9.7
[1.9.6]: https://github.com/glittercowboy/core/releases/tag/v1.9.6
[1.9.5]: https://github.com/glittercowboy/core/releases/tag/v1.9.5
[1.9.4]: https://github.com/glittercowboy/core/releases/tag/v1.9.4
[1.9.2]: https://github.com/glittercowboy/core/releases/tag/v1.9.2
[1.9.0]: https://github.com/glittercowboy/core/releases/tag/v1.9.0
[1.8.0]: https://github.com/glittercowboy/core/releases/tag/v1.8.0
[1.7.1]: https://github.com/glittercowboy/core/releases/tag/v1.7.1
[1.7.0]: https://github.com/glittercowboy/core/releases/tag/v1.7.0
[1.6.4]: https://github.com/glittercowboy/core/releases/tag/v1.6.4
[1.6.3]: https://github.com/glittercowboy/core/releases/tag/v1.6.3
[1.6.2]: https://github.com/glittercowboy/core/releases/tag/v1.6.2
[1.6.1]: https://github.com/glittercowboy/core/releases/tag/v1.6.1
[1.6.0]: https://github.com/glittercowboy/core/releases/tag/v1.6.0
[1.5.30]: https://github.com/glittercowboy/core/releases/tag/v1.5.30
[1.5.29]: https://github.com/glittercowboy/core/releases/tag/v1.5.29
[1.5.28]: https://github.com/glittercowboy/core/releases/tag/v1.5.28
[1.5.27]: https://github.com/glittercowboy/core/releases/tag/v1.5.27
[1.5.26]: https://github.com/glittercowboy/core/releases/tag/v1.5.26
[1.5.25]: https://github.com/glittercowboy/core/releases/tag/v1.5.25
[1.5.24]: https://github.com/glittercowboy/core/releases/tag/v1.5.24
[1.5.23]: https://github.com/glittercowboy/core/releases/tag/v1.5.23
[1.5.22]: https://github.com/glittercowboy/core/releases/tag/v1.5.22
[1.5.21]: https://github.com/glittercowboy/core/releases/tag/v1.5.21
[1.5.20]: https://github.com/glittercowboy/core/releases/tag/v1.5.20
[1.5.19]: https://github.com/glittercowboy/core/releases/tag/v1.5.19
[1.5.18]: https://github.com/glittercowboy/core/releases/tag/v1.5.18
[1.5.17]: https://github.com/glittercowboy/core/releases/tag/v1.5.17
[1.5.16]: https://github.com/glittercowboy/core/releases/tag/v1.5.16
[1.5.15]: https://github.com/glittercowboy/core/releases/tag/v1.5.15
[1.5.14]: https://github.com/glittercowboy/core/releases/tag/v1.5.14
[1.5.13]: https://github.com/glittercowboy/core/releases/tag/v1.5.13
[1.5.12]: https://github.com/glittercowboy/core/releases/tag/v1.5.12
[1.5.11]: https://github.com/glittercowboy/core/releases/tag/v1.5.11
[1.5.10]: https://github.com/glittercowboy/core/releases/tag/v1.5.10
[1.5.9]: https://github.com/glittercowboy/core/releases/tag/v1.5.9
[1.5.8]: https://github.com/glittercowboy/core/releases/tag/v1.5.8
[1.5.7]: https://github.com/glittercowboy/core/releases/tag/v1.5.7
[1.5.6]: https://github.com/glittercowboy/core/releases/tag/v1.5.6
[1.5.5]: https://github.com/glittercowboy/core/releases/tag/v1.5.5
[1.5.4]: https://github.com/glittercowboy/core/releases/tag/v1.5.4
[1.5.3]: https://github.com/glittercowboy/core/releases/tag/v1.5.3
[1.5.2]: https://github.com/glittercowboy/core/releases/tag/v1.5.2
[1.5.1]: https://github.com/glittercowboy/core/releases/tag/v1.5.1
[1.5.0]: https://github.com/glittercowboy/core/releases/tag/v1.5.0
[1.4.29]: https://github.com/glittercowboy/core/releases/tag/v1.4.29
[1.4.28]: https://github.com/glittercowboy/core/releases/tag/v1.4.28
[1.4.27]: https://github.com/glittercowboy/core/releases/tag/v1.4.27
[1.4.26]: https://github.com/glittercowboy/core/releases/tag/v1.4.26
[1.4.25]: https://github.com/glittercowboy/core/releases/tag/v1.4.25
[1.4.24]: https://github.com/glittercowboy/core/releases/tag/v1.4.24
[1.4.23]: https://github.com/glittercowboy/core/releases/tag/v1.4.23
[1.4.22]: https://github.com/glittercowboy/core/releases/tag/v1.4.22
[1.4.21]: https://github.com/glittercowboy/core/releases/tag/v1.4.21
[1.4.20]: https://github.com/glittercowboy/core/releases/tag/v1.4.20
[1.4.19]: https://github.com/glittercowboy/core/releases/tag/v1.4.19
[1.4.18]: https://github.com/glittercowboy/core/releases/tag/v1.4.18
[1.4.17]: https://github.com/glittercowboy/core/releases/tag/v1.4.17
[1.4.16]: https://github.com/glittercowboy/core/releases/tag/v1.4.16
[1.4.15]: https://github.com/glittercowboy/core/releases/tag/v1.4.15
[1.4.14]: https://github.com/glittercowboy/core/releases/tag/v1.4.14
[1.4.13]: https://github.com/glittercowboy/core/releases/tag/v1.4.13
[1.4.12]: https://github.com/glittercowboy/core/releases/tag/v1.4.12
[1.4.11]: https://github.com/glittercowboy/core/releases/tag/v1.4.11
[1.4.10]: https://github.com/glittercowboy/core/releases/tag/v1.4.10
[1.4.9]: https://github.com/glittercowboy/core/releases/tag/v1.4.9
[1.4.8]: https://github.com/glittercowboy/core/releases/tag/v1.4.8
[1.4.7]: https://github.com/glittercowboy/core/releases/tag/v1.4.7
[1.4.6]: https://github.com/glittercowboy/core/releases/tag/v1.4.6
[1.4.5]: https://github.com/glittercowboy/core/releases/tag/v1.4.5
[1.4.4]: https://github.com/glittercowboy/core/releases/tag/v1.4.4
[1.4.3]: https://github.com/glittercowboy/core/releases/tag/v1.4.3
[1.4.2]: https://github.com/glittercowboy/core/releases/tag/v1.4.2
[1.4.1]: https://github.com/glittercowboy/core/releases/tag/v1.4.1
[1.4.0]: https://github.com/glittercowboy/core/releases/tag/v1.4.0
[1.3.34]: https://github.com/glittercowboy/core/releases/tag/v1.3.34
[1.3.33]: https://github.com/glittercowboy/core/releases/tag/v1.3.33
[1.3.32]: https://github.com/glittercowboy/core/releases/tag/v1.3.32
[1.3.31]: https://github.com/glittercowboy/core/releases/tag/v1.3.31
[1.3.30]: https://github.com/glittercowboy/core/releases/tag/v1.3.30
[1.3.29]: https://github.com/glittercowboy/core/releases/tag/v1.3.29
[1.3.28]: https://github.com/glittercowboy/core/releases/tag/v1.3.28
[1.3.27]: https://github.com/glittercowboy/core/releases/tag/v1.3.27
[1.3.26]: https://github.com/glittercowboy/core/releases/tag/v1.3.26
[1.3.25]: https://github.com/glittercowboy/core/releases/tag/v1.3.25
[1.3.24]: https://github.com/glittercowboy/core/releases/tag/v1.3.24
[1.3.23]: https://github.com/glittercowboy/core/releases/tag/v1.3.23
[1.3.22]: https://github.com/glittercowboy/core/releases/tag/v1.3.22
[1.3.21]: https://github.com/glittercowboy/core/releases/tag/v1.3.21
[1.3.20]: https://github.com/glittercowboy/core/releases/tag/v1.3.20
[1.3.19]: https://github.com/glittercowboy/core/releases/tag/v1.3.19
[1.3.18]: https://github.com/glittercowboy/core/releases/tag/v1.3.18
[1.3.17]: https://github.com/glittercowboy/core/releases/tag/v1.3.17
[1.3.16]: https://github.com/glittercowboy/core/releases/tag/v1.3.16
[1.3.15]: https://github.com/glittercowboy/core/releases/tag/v1.3.15
[1.3.14]: https://github.com/glittercowboy/core/releases/tag/v1.3.14
[1.3.13]: https://github.com/glittercowboy/core/releases/tag/v1.3.13
[1.3.12]: https://github.com/glittercowboy/core/releases/tag/v1.3.12
[1.3.11]: https://github.com/glittercowboy/core/releases/tag/v1.3.11
[1.3.10]: https://github.com/glittercowboy/core/releases/tag/v1.3.10
[1.3.9]: https://github.com/glittercowboy/core/releases/tag/v1.3.9
[1.3.8]: https://github.com/glittercowboy/core/releases/tag/v1.3.8
[1.3.7]: https://github.com/glittercowboy/core/releases/tag/v1.3.7
[1.3.6]: https://github.com/glittercowboy/core/releases/tag/v1.3.6
[1.3.5]: https://github.com/glittercowboy/core/releases/tag/v1.3.5
[1.3.4]: https://github.com/glittercowboy/core/releases/tag/v1.3.4
[1.3.3]: https://github.com/glittercowboy/core/releases/tag/v1.3.3
[1.3.2]: https://github.com/glittercowboy/core/releases/tag/v1.3.2
[1.3.1]: https://github.com/glittercowboy/core/releases/tag/v1.3.1
[1.3.0]: https://github.com/glittercowboy/core/releases/tag/v1.3.0
[1.2.13]: https://github.com/glittercowboy/core/releases/tag/v1.2.13
[1.2.12]: https://github.com/glittercowboy/core/releases/tag/v1.2.12
[1.2.11]: https://github.com/glittercowboy/core/releases/tag/v1.2.11
[1.2.10]: https://github.com/glittercowboy/core/releases/tag/v1.2.10
[1.2.9]: https://github.com/glittercowboy/core/releases/tag/v1.2.9
[1.2.8]: https://github.com/glittercowboy/core/releases/tag/v1.2.8
[1.2.7]: https://github.com/glittercowboy/core/releases/tag/v1.2.7
[1.2.6]: https://github.com/glittercowboy/core/releases/tag/v1.2.6
[1.2.5]: https://github.com/glittercowboy/core/releases/tag/v1.2.5
[1.2.4]: https://github.com/glittercowboy/core/releases/tag/v1.2.4
[1.2.3]: https://github.com/glittercowboy/core/releases/tag/v1.2.3
[1.2.2]: https://github.com/glittercowboy/core/releases/tag/v1.2.2
[1.2.1]: https://github.com/glittercowboy/core/releases/tag/v1.2.1
[1.2.0]: https://github.com/glittercowboy/core/releases/tag/v1.2.0
[1.1.2]: https://github.com/glittercowboy/core/releases/tag/v1.1.2
[1.1.1]: https://github.com/glittercowboy/core/releases/tag/v1.1.1
[1.1.0]: https://github.com/glittercowboy/core/releases/tag/v1.1.0
[1.0.11]: https://github.com/glittercowboy/core/releases/tag/v1.0.11
[1.0.10]: https://github.com/glittercowboy/core/releases/tag/v1.0.10
[1.0.9]: https://github.com/glittercowboy/core/releases/tag/v1.0.9
[1.0.8]: https://github.com/glittercowboy/core/releases/tag/v1.0.8
[1.0.7]: https://github.com/glittercowboy/core/releases/tag/v1.0.7
[1.0.6]: https://github.com/glittercowboy/core/releases/tag/v1.0.6
[1.0.5]: https://github.com/glittercowboy/core/releases/tag/v1.0.5
[1.0.4]: https://github.com/glittercowboy/core/releases/tag/v1.0.4
[1.0.3]: https://github.com/glittercowboy/core/releases/tag/v1.0.3
[1.0.2]: https://github.com/glittercowboy/core/releases/tag/v1.0.2
[1.0.1]: https://github.com/glittercowboy/core/releases/tag/v1.0.1
[1.0.0]: https://github.com/glittercowboy/core/releases/tag/v1.0.0
