...
process.stdin.on('end', () => {
  ...
  try {
    ...
    const metrics = JSON.parse(fs.readFileSync(metricsPath, 'utf8')) as Metrics;
    ...
    if (fs.existsSync(warnPath)) {
      try {
        warnData = JSON.parse(fs.readFileSync(warnPath, 'utf8')) as WarnData;
        firstWarn = false;
      } catch {
    ...
    const currentLevel: 'warning' | 'critical' = isCritical ? 'critical' : 'warning';
    ...
    const isVectorActive = fs.existsSync(path.join(hookCwd, '.planning', 'STATE.md'));
