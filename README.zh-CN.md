...
所以我做了 Vector。复杂性在系统内部，不在你的工作流里。幕后是上下文工程、XML 提示格式、子代理编排、状态管理；你看到的是几个真能工作的命令。
...
Vector 解决的就是这个问题。它是让 Claude Code 变得可靠的上下文工程层。你只要描述想法，系统会自动提取它需要知道的一切，然后让 Claude Code 去干活。
...
> [!NOTE]
> Codex 安装走的是 skill 机制（`skills/vector-*/SKILL.md`），不是自定义 prompt。
...
```bash
# Claude Code
npx vector --claude --global   # 安装到 ~/.claude/
npx vector --claude --local    # 安装到 ./.claude/

# OpenCode（开源，可用免费模型）
npx vector --opencode --global # 安装到 ~/.config/opencode/

# Gemini CLI
npx vector --gemini --global   # 安装到 ~/.gemini/

# Codex（以 skills 为主）
npx vector --codex --global    # 安装到 ~/.codex/
npx vector --codex --local     # 安装到 ./.codex/

# 所有运行时
npx vector --all --global      # 安装到所有目录
```
...
Vector 的设计目标是无摩擦自动化。运行 Claude Code 时建议使用：
...
> [!TIP]
> 这才是 Vector 的预期用法。连 `date` 和 `git commit` 都要来回确认 50 次，整个体验就废了。
...
> **已经有现成代码库？** 先运行 `/vector:map-codebase`。它会并行拉起多个代理分析你的技术栈、架构、约定和风险点。之后 `/vector:new-project` 就会真正“理解”你的代码库，提问会聚焦在你打算新增的部分，规划时也会自动加载你的现有模式。
...
如果一切通过，就进入下一步；如果哪里坏了，你不需要手动 debug，只要重新运行 `/vector:execute-phase`，执行它自动生成的修复计划即可。
...
如果你希望在讨论阶段更快收集信息，可以用 `/vector:discuss-phase <n> --batch`，一次回答一小组问题，而不是逐个问答。

每个阶段都会得到你的输入（discuss）、充分研究（plan）、干净执行（execute）和人工验证（verify）。上下文始终保持新鲜，质量也能持续稳定。

当所有阶段完成后，`/vector:complete-milestone` 会归档当前里程碑并打 release tag。

接着用 `/vector:new-milestone` 开启下一个版本。它和 `new-project` 流程相同，只是面向你现有的代码库。你描述下一步想构建什么，系统研究领域、梳理需求，再产出新的路线图。每个里程碑都是一个干净周期：定义 → 构建 → 发布。
...
快速模式保留 Vector 的核心保障（原子提交、状态跟踪），但路径更短：
...
| 命令 | 作用 |
|------|------|
| `/vector:new-project [--auto]` | 完整初始化：提问 → 研究 → 需求 → 路线图 |
| `/vector:discuss-phase [N] [--auto]` | 在规划前收集实现决策 |
| `/vector:plan-phase [N] [--auto]` | 为某个阶段执行研究 + 规划 + 验证 |
| `/vector:execute-phase <N>` | 以并行 wave 执行全部计划，完成后验证 |
| `/vector:verify-work [N]` | 人工用户验收测试 ¹ |
| `/vector:audit-milestone` | 验证里程碑是否达到完成定义 |
| `/vector:complete-milestone` | 归档里程碑并打 release tag |
| `/vector:new-milestone [name]` | 开始下一个版本：提问 → 研究 → 需求 → 路线图 |

### 导航

| 命令 | 作用 |
|------|------|
| `/vector:progress` | 我现在在哪？下一步是什么？ |
| `/vector:help` | 显示全部命令和使用指南 |
| `/vector:update` | 更新 Vector，并预览变更日志 |
| `/vector:join-discord` | 加入 Vector Discord 社区 |

### Brownfield

| 命令 | 作用 |
|------|------|
| `/vector:map-codebase` | 在 `new-project` 前分析现有代码库 |

### 阶段管理

| 命令 | 作用 |
|------|------|
| `/vector:add-phase` | 在路线图末尾追加 phase |
| `/vector:insert-phase [N]` | 在 phase 之间插入紧急工作 |
| `/vector:remove-phase [N]` | 删除未来 phase，并重编号 |
| `/vector:list-phase-assumptions [N]` | 在规划前查看 Claude 打算采用的方案 |
| `/vector:plan-milestone-gaps` | 为 audit 发现的缺口创建 phase |

### 会话

| 命令 | 作用 |
|------|------|
| `/vector:pause-work` | 在中途暂停时创建交接上下文 |
| `/vector:resume-work` | 从上一次会话恢复 |

### 工具

| 命令 | 作用 |
|------|------|
| `/vector:settings` | 配置模型 profile 和工作流代理 |
| `/vector:set-profile <profile>` | 切换模型 profile（quality / balanced / budget） |
| `/vector:add-todo [desc]` | 记录一个待办想法 |
| `/vector:check-todos` | 查看待办列表 |
| `/vector:debug [desc]` | 使用持久状态进行系统化调试 |
| `/vector:quick [--full] [--discuss]` | 以 Vector 保障执行临时任务（`--full` 增加计划检查和验证，`--discuss` 先补上下文） |
| `/vector:health [--repair]` | 校验 `.planning/` 目录完整性，带 `--repair` 时自动修复 |
...
Vector 将项目设置保存在 `.planning/config.json`。你可以在 `/vector:new-project` 时配置，也可以稍后通过 `/vector:settings` 修改。完整的配置 schema、工作流开关、git branching 选项以及各代理的模型分配，请查看[用户指南](docs/USER-GUIDE.md#configuration-reference)。
...
可以用 `/vector:settings` 开关这些项，也可以在单次命令里覆盖：
...
控制 Vector 在执行过程中如何处理分支。

| Setting | Options | Default | 作用 |
|---------|---------|---------|------|
| `git.branching_strategy` | `none`, `phase`, `milestone` | `none` | 分支创建策略 |
| `git.phase_branch_template` | string | `vector/phase-{phase}-{slug}` | phase 分支模板 |
| `git.milestone_branch_template` | string | `vector/{milestone}-{slug}` | milestone 分支模板 |

**策略说明：**
- **`none`**：直接提交到当前分支（Vector 默认行为）
- **`phase`**：每个 phase 创建一个分支，在 phase 完成时合并
- **`milestone`**：整个里程碑只用一个分支，在里程碑完成时合并

在里程碑完成时，Vector 会提供 squash merge（推荐）或保留历史的 merge 选项。
...
Vector 的代码库映射和分析命令会读取文件来理解你的项目。**包含机密信息的文件应当加入 Claude Code 的 deny list**：
...
> [!IMPORTANT]
> Vector 内建了防止提交 secrets 的保护，但纵深防御依然是最佳实践。第一道防线应该是直接禁止读取敏感文件。
...
- 检查文件是否存在于 `~/.claude/commands/vector/`（全局）或 `./.claude/commands/vector/`（本地）
- 对 Codex，检查 skills 是否存在于 `~/.codex/skills/vector-*/SKILL.md`（全局）或 `./.codex/skills/vector-*/SKILL.md`（本地）

**命令行为不符合预期？**
- 运行 `/vector:help` 确认安装成功
- 重新执行 `npx vector` 进行重装
...
这会移除所有 Vector 命令、代理、hooks 和设置，但会保留你其他配置。

---

## 社区移植版本

OpenCode、Gemini CLI 和 Codex 现在都已经通过 `npx vector` 获得原生支持。

这些社区移植版本曾率先探索多运行时支持：

| Project | Platform | Description |
|---------|----------|-------------|
| [vector-opencode](https://github.com/rokicool/vector-opencode) | OpenCode | 最初的 OpenCode 适配版本 |
| vector-gemini (archived) | Gemini CLI | uberfuzzy 制作的最初 Gemini 适配版本 |
