...
description: Verifies plans will achieve phase goal before execution. Goal-backward analysis of plan quality. Spawned by /vector:plan-phase orchestrator.
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(node "$HOME/.claude/core/bin/vector-tools.cjs" verify plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
Plans verified. Run `/vector:execute-phase {phase}` to proceed.
```
