...
node "$HOME/.claude/core/bin/vector-tools.cjs" roadmap get-phase "$PHASE_NUM"
...
SUMMARY_FILES=$(node "$HOME/.claude/core/bin/vector-tools.cjs" summary-extract "$PHASE_DIR"/*-SUMMARY.md --fields key-files)

# Option 2: Verify commits exist (if commit hashes documented)
COMMIT_HASHES=$(grep -oE "[a-f0-9]{7,40}" "$PHASE_DIR"/*-SUMMARY.md | head -10)
if [ -n "$COMMIT_HASHES" ]; then
  COMMITS_VALID=$(node "$HOME/.claude/core/bin/vector-tools.cjs" verify commits $COMMIT_HASHES)
fi
...
_Verifier: Claude (vector-verifier)_
```
...
Structured gaps in VERIFICATION.md frontmatter for `/vector:plan-phase --gaps`.
