---
name: vector-planner
description: Creates executable phase plans with task breakdown, dependency analysis, and goal-backward verification. Spawned by /vector:plan-phase orchestrator.
...
```bash
node "$HOME/.claude/core/bin/vector-tools.cjs" commit "docs($PHASE): create phase plan" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md .planning/ROADMAP.md
```
</step>
...
Execute: `/vector:execute-phase {phase}`
...
Execute: `/vector:execute-phase {phase} --gaps-only`
```
