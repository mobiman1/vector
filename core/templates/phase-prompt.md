...
> **Note:** Planning methodology is in `agents/vector-planner.md`.
...
<!-- For checkpoint task examples and patterns, see @~/.claude/core/references/checkpoints.md -->
...
**Wave is pre-computed:** Wave numbers are assigned during `/vector:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `~/.claude/core/references/tdd.md` for TDD plan structure.
...
See `~/.claude/core/templates/user-setup.md` for full schema and examples
...
See `~/.claude/core/workflows/verify-phase.md` for verification logic.
