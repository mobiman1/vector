...
import { error } from './lib/core.cjs';
import * as state from './lib/state.cjs';
import * as phase from './lib/phase.cjs';
import * as roadmap from './lib/roadmap.cjs';
import * as verify from './lib/verify.cjs';
import * as config from './lib/config.cjs';
import * as template from './lib/template.cjs';
import * as milestone from './lib/milestone.cjs';
import * as commands from './lib/commands.cjs';
import * as init from './lib/init.cjs';
import * as frontmatter from './lib/frontmatter.cjs';
...
async function main(): Promise<void> {
  ...
  if (!command) {
    error('Usage: vector-tools <command> [args] [--raw] [--cwd <path>]\nCommands: state, resolve-model, find-phase, commit, verify-summary, verify, frontmatter, template, generate-slug, current-timestamp, list-todos, verify-path-exists, config-ensure-section, init');
  }
    ...
    case 'template': {
      ...
      if (subcommand === 'select') {
        ...
        template.cmdTemplateFill(cwd, templateType, {
          ...
          fields: fieldsIdx !== -1 ? JSON.parse(args[fieldsIdx + 1]) as Record<string, unknown> : {},
        }, raw);
      } else {
