...
exports.searchPhaseInDir = searchPhaseInDir;
exports.findPhaseInternal = findPhaseInternal;
exports.getArchivedPhaseDirs = getArchivedPhaseDirs;
exports.stripShippedMilestones = stripShippedMilestones;
exports.replaceInCurrentMilestone = replaceInCurrentMilestone;
...
exports.getMilestonePhaseFilter = getMilestonePhaseFilter;
...
function output(result, raw, rawValue) {
    if (raw && rawValue !== undefined) {
        ...
        if (json.length > 50000) {
            const tmpPath = path_1.default.join(os_1.default.tmpdir(), `vector-${Date.now()}.json`);
            fs_1.default.writeFileSync(tmpPath, json, 'utf-8');
...
function loadConfig(cwd) {
    ...
    try {
        ...
        if ('depth' in parsed && !('granularity' in parsed)) {
            ...
            delete parsed.depth;
            try {
                fs_1.default.writeFileSync(configPath, JSON.stringify(parsed, null, 2), 'utf-8');
            }
            catch { }
        }
...
function isGitIgnored(cwd, targetPath) {
    try {
        ...
        (0, child_process_1.execSync)('git check-ignore -q --no-index -- ' + targetPath.replace(/[^a-zA-Z0-9._\-/]/g, ''), {
            cwd,
            stdio: 'pipe',
        });
        return true;
...
function searchPhaseInDir(baseDir, relBase, normalized) {
    try {
        ...
        const phaseDir = path_1.default.join(baseDir, match);
        ...
        return {
            found: true,
            directory: toPosixPath(path_1.default.join(relBase, match)),
...
function findPhaseInternal(cwd, phase) {
    ...
    const milestonesDir = path_1.default.join(cwd, '.planning', 'milestones');
    if (!fs_1.default.existsSync(milestonesDir))
        return null;
    try {
        const milestoneEntries = fs_1.default.readdirSync(milestonesDir, { withFileTypes: true });
        ...
        for (const archiveName of archiveDirs) {
            ...
            const archivePath = path_1.default.join(milestonesDir, archiveName);
...
function getArchivedPhaseDirs(cwd) {
    ...
    try {
        ...
        for (const archiveName of phaseDirs) {
            ...
            const version = versionMatch ? versionMatch[1] : archiveName;
            const archivePath = path_1.default.join(milestonesDir, archiveName);
            const entries = fs_1.default.readdirSync(archivePath, { withFileTypes: true });
            const dirs = entries.filter(e => e.isDirectory()).map(e => e.name).sort((a, b) => comparePhaseNum(a, b));
            for (const dir of dirs) {
                results.push({
                    name: dir,
                    milestone: version,
                    basePath: path_1.default.join('.planning', 'milestones', archiveName),
                    fullPath: path_1.default.join(archivePath, dir),
                });
            }
        }
    }
...
function getRoadmapPhaseInternal(cwd, phaseNum) {
    ...
    if (!fs_1.default.existsSync(roadmapPath))
        return null;
    try {
        const content = stripShippedMilestones(fs_1.default.readFileSync(roadmapPath, 'utf-8'));
...
function pathExistsInternal(cwd, targetPath) {
    ...
    try {
        fs_1.default.statSync(fullPath);
        return true;
    }
    catch {
        return false;
...
function getMilestoneInfo(cwd) {
    try {
        const roadmap = fs_1.default.readFileSync(path_1.default.join(cwd, '.planning', 'ROADMAP.md'), 'utf-8');
...
function getMilestonePhaseFilter(cwd) {
    ...
    try {
        const roadmap = stripShippedMilestones(fs_1.default.readFileSync(path_1.default.join(cwd, '.planning', 'ROADMAP.md'), 'utf-8'));
