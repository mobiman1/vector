...
import { escapeRegex, loadConfig, getMilestoneInfo, getMilestonePhaseFilter, output, error, PhaseFilter } from './core.cjs';
import { extractFrontmatter, reconstructFrontmatter } from './frontmatter.cjs';
...
function readTextArgOrFile(cwd: string, value: string | null | undefined, filePath: string | null | undefined, label: string): string | null {
...
export function cmdStateUpdateProgress(cwd: string, raw: boolean): void {
  ...
  if (fs.existsSync(phasesDir)) {
    const isDirInMilestone: PhaseFilter = getMilestonePhaseFilter(cwd);
...
export function cmdStateAddBlocker(cwd: string, text: string | AddBlockerOptions, raw: boolean): void {
  const statePath = path.join(cwd, '.planning', 'STATE.md');
  if (!fs.existsSync(statePath)) { output({ error: 'STATE.md not found' }, raw); return; }
  const blockerOptions: AddBlockerOptions = typeof text === 'object' && text !== null ? text : { text: text as string };
...
export function cmdStateResolveBlocker(cwd: string, text: string | null | undefined, raw: boolean): void {
  const statePath = path.join(cwd, '.planning', 'STATE.md');
  if (!fs.existsSync(statePath)) { output({ error: 'STATE.md not found' }, raw); return; }
 ...
 */
function buildStateFrontmatter(bodyContent: string, cwd: string | null): Record<string, unknown> {
  ...
  const fm: Record<string, unknown> = { vector_state_version: '1.0' };
