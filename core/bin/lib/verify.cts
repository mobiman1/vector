...
import { safeReadFile, normalizePhaseName, execGit, findPhaseInternal, getMilestoneInfo, output, error } from './core.cjs';
import { extractFrontmatter, parseMustHavesBlock } from './frontmatter.cjs';
import { writeStateMd } from './state.cjs';

export function cmdVerifySummary(cwd: string, summaryPath: string | undefined, checkFileCount: number | undefined, raw: boolean): void {
  if (!summaryPath) {
...
export function cmdVerifyPlanStructure(cwd: string, filePath: string | undefined, raw: boolean): void {
  ...
  if (fm.wave && parseInt(fm.wave as string) > 1 && (!fm.depends_on || (Array.isArray(fm.depends_on) && fm.depends_on.length === 0))) {
...
export function cmdVerifyPhaseCompleteness(cwd: string, phase: string | undefined, raw: boolean): void {
...
export function cmdVerifyReferences(cwd: string, filePath: string | undefined, raw: boolean): void {
  if (!filePath) { error('file path required'); }
  const fullPath = path.isAbsolute(filePath!) ? filePath! : path.join(cwd, filePath!);
  const content = safeReadFile(fullPath);
  if (!content) { output({ error: 'File not found', path: filePath }, raw); return; }
...
export function cmdVerifyArtifacts(cwd: string, planFilePath: string | undefined, raw: boolean): void {
  ...
  for (const artifact of artifacts) {
    ...
    const check: { path: string; exists: boolean; issues: string[]; passed: boolean } = { path: artPath, exists, issues: [], passed: false };
    ...
    if (exists) {
      ...
      if (artObj.exports) {
        const exports = Array.isArray(artObj.exports) ? artObj.exports as string[] : [artObj.exports as string];
...
export function cmdVerifyKeyLinks(cwd: string, planFilePath: string | undefined, raw: boolean): void {
  if (!planFilePath) { error('plan file path required'); }
  const fullPath = path.isAbsolute(planFilePath!) ? planFilePath! : path.join(cwd, planFilePath!);
  const content = safeReadFile(fullPath);
  if (!content) { output({ error: 'File not found', path: planFilePath }, raw); return; }
  ...
  for (const link of keyLinks) {
    ...
    const check = { from: linkObj.from as string, to: linkObj.to as string, via: (linkObj.via as string) || '', verified: false, detail: '' };

    const sourceContent = safeReadFile(path.join(cwd, (linkObj.from as string) || ''));
    if (!sourceContent) {
      ...
      try {
        ...
        if (regex.test(sourceContent)) {
          ...
          const targetContent = safeReadFile(path.join(cwd, (linkObj.to as string) || ''));
      ...
      if (sourceContent.includes((linkObj.to as string) || '')) {
        check.verified = true;
...
export function cmdValidateHealth(cwd: string, options: HealthOptions, raw: boolean): void {
  ...
  if (!fs.existsSync(configPath)) {
    ...
    try {
      ...
      addIssue('error', 'E005', `config.json: JSON parse error - ${(err as Error).message}`, 'Run /vector:health --repair to reset to defaults', true);
