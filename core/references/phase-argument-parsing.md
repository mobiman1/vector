...
```bash
PHASE_INFO=$(node "$HOME/.claude/core/bin/vector-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "$HOME/.claude/core/bin/vector-tools.cjs" roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node "$HOME/.claude/core/bin/vector-tools.cjs" find-phase "${PHASE}" --raw)
```
