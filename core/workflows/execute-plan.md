...
@~/.claude/core/references/git-integration.md
</required_reading>
...
# config settings can be fetched via vector-tools config-get if needed
```
...
**Pattern A:** init_agent_tracking → spawn Task(subagent_type="vector-executor", model=executor_model) with prompt: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash → track agent_id → wait → update tracking → report.
...
2. Per segment:
   - Subagent route: spawn vector-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
```bash
node "$HOME/.claude/core/bin/vector-tools.cjs" phases list --type summaries --raw
...
See `~/.claude/core/references/tdd.md` for structure.
</tdd_plan_execution>
...
See ~/.claude/core/references/checkpoints.md for details.
</step>
...
```bash
NODE_REPAIR=$(node "./.claude/core/bin/vector-tools.cjs" config-get workflow.node_repair 2>/dev/null || echo "true")
```

If `NODE_REPAIR` is `true`: invoke `@./.claude/core/workflows/node-repair.md` with:
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/core/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `~/.claude/core/templates/summary.md`.
...
Update STATE.md using vector-tools:
...
node "$HOME/.claude/core/bin/vector-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>

<step name="update_session_continuity">
Update session info using vector-tools:
...
```bash
node "$HOME/.claude/core/bin/vector-tools.cjs" roadmap update-plan-progress "${PHASE}"
```
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/vector:execute-phase {phase}` + `/vector:verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/vector:plan-phase {Z+1}` + `/vector:verify-work {Z}` + `/vector:discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/vector:complete-milestone` + `/vector:verify-work` + `/vector:add-phase` |
