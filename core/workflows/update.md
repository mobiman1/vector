<purpose>
Check for Vector updates via npm, display changelog for versions between installed and latest, obtain user confirmation, and execute clean installation with cache clearing.
</purpose>
...
Detect whether Vector is installed locally or globally by checking both locations and validating install integrity.
...
Use `PREFERRED_RUNTIME` as the first runtime checked so `/vector:update` targets the runtime that invoked it.
...
for entry in $ORDERED_RUNTIME_DIRS; do
  runtime="${entry%%:*}"
  dir="${entry#*:}"
  if [ -f "./$dir/core/VERSION" ] || [ -f "./$dir/core/workflows/update.md" ]; then
    LOCAL_RUNTIME="$runtime"
    LOCAL_VERSION_FILE="./$dir/core/VERSION"
    LOCAL_MARKER_FILE="./$dir/core/workflows/update.md"
...
for entry in $ORDERED_RUNTIME_DIRS; do
  runtime="${entry%%:*}"
  dir="${entry#*:}"
  if [ -f "$HOME/$dir/core/VERSION" ] || [ -f "$HOME/$dir/core/workflows/update.md" ]; then
    GLOBAL_RUNTIME="$runtime"
    GLOBAL_VERSION_FILE="$HOME/$dir/core/VERSION"
    GLOBAL_MARKER_FILE="$HOME/$dir/core/workflows/update.md"
...
```bash
npm view vector version 2>/dev/null
```
...
To update manually: `npx vector --global`
```
...
⚠️  **Note:** The installer performs a clean install of Vector folders:
- `commands/vector/` will be wiped and replaced
- `core/` will be wiped and replaced
- `agents/vector-*` files will be replaced
...
- Custom commands not in `commands/vector/` ✓
- Custom agents not prefixed with `vector-` ✓
- Custom hooks ✓
- Your CLAUDE.md files ✓

If you've modified any Vector files directly, they'll be automatically backed up to `vector-local-patches/` and can be reapplied with `/vector:reapply-patches` after the update.
```
...
**If LOCAL install:**
```bash
npx -y vector@latest "$RUNTIME_FLAG" --local
```

**If GLOBAL install:**
```bash
npx -y vector@latest "$RUNTIME_FLAG" --global
```

**If UNKNOWN install:**
```bash
npx -y vector@latest --claude --global
```
...
for dir in .claude .config/opencode .opencode .gemini .codex; do
  rm -f "./$dir/cache/vector-update-check.json"
  rm -f "$HOME/$dir/cache/vector-update-check.json"
done
```

The SessionStart hook (`vector-check-update.js`) writes to the detected runtime's cache directory, so all paths must be cleared to prevent stale update indicators.
</step>
...
Check for vector-local-patches/backup-meta.json in the config directory.

**If patches found:**

```
Local patches were backed up before the update.
Run /vector:reapply-patches to merge your modifications into the new version.
```
