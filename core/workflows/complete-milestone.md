...
2. **Run audit first** — `/vector:audit-milestone` to assess gap severity
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node "$HOME/.claude/core/bin/vector-tools.cjs" summary-extract "$summary" --fields one_liner | jq -r '.one_liner'
done
```
...
**Note:** MILESTONES.md entry is now created automatically by `vector-tools milestone complete` in the archive_milestone step. The entry includes version, date, phase/plan/task counts, and accomplishments extracted from SUMMARY.md files.
...
**Delegate archival to vector-tools:**

```bash
ARCHIVE=$(node "$HOME/.claude/core/bin/vector-tools.cjs" milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
If "Skip": Phase directories remain in `.planning/phases/` as raw execution history. Use `/vector:cleanup` later to archive retroactively.
...
**If doesn't exist:** Create from template at `~/.claude/core/templates/retrospective.md`.
...
**Commit:**
```bash
node "$HOME/.claude/core/bin/vector-tools.cjs" commit "docs: update retrospective for v${VERSION}" --files .planning/RETROSPECTIVE.md
```
...
```bash
INIT=$(node "$HOME/.claude/core/bin/vector-tools.cjs" init execute-phase "1")
...
```bash
node "$HOME/.claude/core/bin/vector-tools.cjs" commit "chore: complete v[X.Y] milestone" --files .planning/milestones/v[X.Y]-ROADMAP.md .planning/milestones/v[X.Y]-REQUIREMENTS.md .planning/milestones/v[X.Y]-MILESTONE-AUDIT.md .planning/MILESTONES.md .planning/PROJECT.md .planning/STATE.md
```
```
...
- [ ] User knows next step (/vector:new-milestone)
