<purpose>
Interactive configuration of Vector workflow agents (research, plan_check, verifier) and model profile selection via multi-question prompt. Updates .planning/config.json with user preferences. Optionally saves settings as global defaults (~/.vector/defaults.json) for future projects.
</purpose>
...
```bash
node "$HOME/.claude/core/bin/vector-tools.cjs" config-ensure-section
...
- `workflow.ui_safety_gate` — prompt to run /vector:ui-phase before planning frontend phases (default: true if absent)
...
AskUserQuestion([
  ...
  {
    question: "Enable UI Safety Gate? (prompts to run /vector:ui-phase before planning frontend phases)",
    header: "UI Gate",
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "plan-phase asks to run /vector:ui-phase first when frontend indicators detected." },
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Per Phase", description: "Create branch for each phase (vector/phase-{N}-{name})" },
      { label: "Per Milestone", description: "Create branch for entire milestone (vector/{version}-{name})" }
    ]
  }
])
```
...
AskUserQuestion([
  {
    ...
    multiSelect: false,
    options: [
      { label: "Yes", description: "New projects start with these settings (saved to ~/.vector/defaults.json)" },
...
If "Yes": write the same config object (minus project-specific fields like `brave_search`) to `~/.vector/defaults.json`:

```bash
mkdir -p ~/.vector
```

Write `~/.vector/defaults.json` with:
```json
{
  "mode": <current>,
  "granularity": <current>,
...
These settings apply to future /vector:plan-phase and /vector:execute-phase runs.

Quick commands:
- /vector:set-profile <profile> — switch model profile
- /vector:plan-phase --research — force research
- /vector:plan-phase --skip-research — skip research
- /vector:plan-phase --skip-verify — skip plan check
```
</step>
...
- [ ] User offered to save as global defaults (~/.vector/defaults.json)
