...
```bash
INIT=$(node "$HOME/.claude/core/bin/vector-tools.cjs" init milestone-op)
```

Parse JSON for: `milestone_version`, `milestone_name`, `phase_count`, `completed_phases`, `roadmap_exists`, `state_exists`, `commit_docs`.

**If `roadmap_exists` is false:** Error — "No ROADMAP.md found. Run `/vector:new-milestone` first."
**If `state_exists` is false:** Error — "No STATE.md found. Run `/vector:new-milestone` first."
...
```bash
ROADMAP=$(node "$HOME/.claude/core/bin/vector-tools.cjs" roadmap analyze)
```
...
```bash
DETAIL=$(node "$HOME/.claude/core/bin/vector-tools.cjs" roadmap get-phase ${PHASE_NUM})
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Vector ► AUTONOMOUS ▸ Phase {N}/{T}: {Name} [████░░░░] {P}%
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
```bash
PHASE_STATE=$(node "$HOME/.claude/core/bin/vector-tools.cjs" init phase-op ${PHASE_NUM})
```
...
```
Skill(skill="vector:plan-phase", args="${PHASE_NUM}")
```
...
```
Skill(skill="vector:execute-phase", args="${PHASE_NUM} --no-transition")
```
...
```
Skill(skill="vector:plan-phase", args="${PHASE_NUM} --gaps")
```
...
> **Note:** Smart discuss is an autonomous-optimized variant of the `vector:discuss-phase` skill. It produces identical CONTEXT.md output but uses batch table proposals instead of sequential questioning. The original `discuss-phase` skill remains unchanged (per CTRL-03). Future milestones may extract this to a separate skill file.
...
```bash
node "$HOME/.claude/core/bin/vector-tools.cjs" commit "docs(${PADDED_PHASE}): smart discuss context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```
Skill(skill="vector:audit-milestone")
```
...
On **"Stop"**: Go to handle_blocker with "User stopped — audit gaps remain. Run /vector:audit-milestone to review, then /vector:complete-milestone when ready."
...
On **"Stop"**: Go to handle_blocker with "User stopped — tech debt to address. Run /vector:audit-milestone to review details."

**5b. Complete Milestone**

```
Skill(skill="vector:complete-milestone", args="${milestone_version}")
```
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Resume with: /vector:autonomous --from {next_phase}
```
...
- [ ] Audit invoked via Skill(skill="vector:audit-milestone")
