...
- Check for MILESTONE-CONTEXT.md (from /vector:discuss-milestone)
...
```bash
node "$HOME/.claude/core/bin/vector-tools.cjs" commit "docs: start milestone v[X.Y] [Name]" --files .planning/PROJECT.md .planning/STATE.md
```

## 7. Load Context and Resolve Models

```bash
INIT=$(node "$HOME/.claude/core/bin/vector-tools.cjs" init new-milestone)
...
**IMPORTANT:** Do NOT persist this choice to config.json. The `workflow.research` setting is a persistent user preference that controls plan-phase behavior across the project. Changing it here would silently alter future `/vector:plan-phase` behavior. To change the default, use `/vector:settings`.
...
Spawn 4 parallel vector-project-researcher agents. Each uses this template with dimension-specific fields:
...
Use template: ~/.claude/core/templates/research-project/{FILE}
</output>
", subagent_type="vector-project-researcher", model="{researcher_model}", description="{DIMENSION} research")
```
...
**Commit requirements:**
```bash
node "$HOME/.claude/core/bin/vector-tools.cjs" commit "docs: define milestone v[X.Y] requirements" --files .planning/REQUIREMENTS.md
```
...
```bash
node "$HOME/.claude/core/bin/vector-tools.cjs" commit "docs: create milestone v[X.Y] roadmap ([N] phases)" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md
```
...
`/vector:discuss-phase [N]` — gather context and clarify approach

<sub>`/clear` first → fresh context window</sub>

Also: `/vector:plan-phase [N]` — skip discussion, plan directly
```
...
- [ ] vector-roadmapper spawned with phase numbering context
- [ ] Roadmap files written immediately (not draft)
- [ ] User feedback incorporated (if any)
- [ ] ROADMAP.md phases continue from previous milestone
- [ ] All commits made (if planning docs committed)
- [ ] User knows next step: `/vector:discuss-phase [N]`
