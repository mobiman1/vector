...
```bash
integration_checker_model=$(node "$HOME/.claude/core/bin/vector-tools.cjs" resolve-model vector-integration-checker --raw)
```
...
node "$HOME/.claude/core/bin/vector-tools.cjs" phases list
```
...
PHASE_INFO=$(node "$HOME/.claude/core/bin/vector-tools.cjs" find-phase 01 --raw)
...
Verify cross-phase wiring and E2E user flows.",
  subagent_type="vector-integration-checker",
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node "$HOME/.claude/core/bin/vector-tools.cjs" summary-extract "$summary" --fields requirements_completed | jq -r '.requirements_completed'
done
```
...
```bash
NYQUIST_CONFIG=$(node "$HOME/.claude/core/bin/vector-tools.cjs" config get workflow.nyquist_validation --raw 2>/dev/null)
```
...
Discovery only — never auto-calls `/vector:validate-phase`.
...
|-------|---------------|-----------|--------|
| {phase} | exists/missing | true/false/partial | `/vector:validate-phase {N}` |

Phases needing validation: run `/vector:validate-phase {N}` for each flagged phase.
...
- /vector:complete-milestone {version} — proceed anyway (accept tech debt)
