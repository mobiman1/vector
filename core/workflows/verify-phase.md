...
<required_reading>
@~/.claude/core/references/verification-patterns.md
@~/.claude/core/templates/verification-report.md
</required_reading>
...
Use vector-tools to extract must_haves from each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node "$HOME/.claude/core/bin/vector-tools.cjs" frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node "$HOME/.claude/core/bin/vector-tools.cjs" roadmap get-phase "${phase_number}" --raw)
```
...
Use vector-tools for artifact verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node "$HOME/.claude/core/bin/vector-tools.cjs" verify artifacts "$plan")
...
Use vector-tools for key link verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node "$HOME/.claude/core/bin/vector-tools.cjs" verify key-links "$plan")
...
See ~/.claude/core/templates/verification-report.md for complete template.
</step>
