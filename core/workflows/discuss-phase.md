...
1. **vector-phase-researcher** — Reads CONTEXT.md to know WHAT to research
   - "User wants card-based layout" → researcher investigates card component patterns
   - "Infinite scroll decided" → researcher looks into virtualization libraries

2. **vector-planner** — Reads CONTEXT.md to know WHAT decisions are locked
...
**Express path available:** If you already have a PRD or acceptance criteria document, use `/vector:plan-phase {phase} --prd path/to/prd.md` to skip this discussion and go straight to planning.
...
Use /vector:progress to see available phases.
```
Exit workflow.
...
- options:
  - "Continue and replan after" — Capture context, then run /vector:plan-phase {X} to replan
...
**Also available:**
- `/vector:plan-phase ${PHASE} --skip-research` — plan without research
- `/vector:ui-phase ${PHASE}` — generate UI design contract before planning (if phase has frontend work)
...
```bash
node "$HOME/.claude/core/bin/vector-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
node "$HOME/.claude/core/bin/vector-tools.cjs" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
...
```
Skill(skill="vector:plan-phase", args="${PHASE} --auto")
```
...
- **PLANNING COMPLETE** → Planning done, execution didn't complete:
  ...
  Continue: /vector:execute-phase ${PHASE}
  ```
- **PLANNING INCONCLUSIVE / CHECKPOINT** → Stop chain:
  ```
  Auto-advance stopped: Planning needs input.
  Continue: /vector:plan-phase ${PHASE}
  ```
- **GAPS FOUND** → Stop chain:
  ```
  Auto-advance stopped: Gaps found during execution.
  Continue: /vector:plan-phase ${PHASE} --gaps
  ```
