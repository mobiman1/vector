<purpose>
Create all phases necessary to close gaps identified by `/vector:audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/vector:add-phase` per gap.
</purpose>
...
```
No audit gaps found. Run `/vector:audit-milestone` first.
```
...
PHASES=$(node "$HOME/.claude/core/bin/vector-tools.cjs" phases list)
...
```bash
node "$HOME/.claude/core/bin/vector-tools.cjs" commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
...
**Also available:**
- `/vector:execute-phase {N}` — if plans already exist
...
`/vector:audit-milestone` — re-audit to verify gaps closed
`/vector:complete-milestone {version}` — archive when audit passes
```
...
- [ ] User knows to run `/vector:plan-phase` next
</success_criteria>
