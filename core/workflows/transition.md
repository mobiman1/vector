...
**Delegate ROADMAP.md and STATE.md updates to vector-tools:**

```bash
TRANSITION=$(node "$HOME/.claude/core/bin/vector-tools.cjs" phase complete "${current_phase}")
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `vector-tools phase complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(node "$HOME/.claude/core/bin/vector-tools.cjs" progress bar --raw)
```
...
**Use the transition result from `vector-tools phase complete`:**
...
Exit skill and invoke SlashCommand("/vector:plan-phase [X+1] --auto")
...
Exit skill and invoke SlashCommand("/vector:discuss-phase [X+1] --auto")
...
`/vector:discuss-phase [X+1]` — gather context and clarify approach
...
**Also available:**
- `/vector:plan-phase [X+1]` — skip discussion, plan directly
- `/vector:research-phase [X+1]` — investigate unknowns
...
**Also available:**
- `/vector:discuss-phase [X+1]` — revisit context
- `/vector:research-phase [X+1]` — investigate unknowns
...
```bash
node "$HOME/.claude/core/bin/vector-tools.cjs" config-set workflow._auto_chain_active false
```
...
Exit skill and invoke SlashCommand("/vector:complete-milestone {version}")
