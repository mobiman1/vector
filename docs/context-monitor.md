...
| CRITICAL | <= 25% | Stop immediately, save state (`/vector:pause-work`) |
...
```
Statusline Hook (vector-statusline.js)
    | writes
    v
/tmp/claude-ctx-{session_id}.json
    ^ reads
    |
Context Monitor (vector-context-monitor.js, PostToolUse/AfterTool)
    | injects
    v
...
Vector's `/vector:pause-work` command saves execution state. The WARNING message suggests using it. The CRITICAL message instructs immediate state save.

## Setup

Both hooks are automatically registered during `npx vector` installation:
...
```json
{
  "statusLine": {
    "type": "command",
    "command": "node ~/.claude/hooks/vector-statusline.js"
  },
  "hooks": {
    "PostToolUse": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "node ~/.claude/hooks/vector-context-monitor.js"
          }
        ]
      }
    ]
    ...
    "AfterTool": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "node ~/.gemini/hooks/vector-context-monitor.js"
          }
        ]
      }
    ]
